...
final class LiarGameSubjectViewController: UIViewController, View {
  ...
  private let animalButton: UIButton = .init()
  private let exerciseButton: UIButton = .init()
  private let foodButton: UIButton = .init()
  private let electronicEquipmentButton: UIButton = .init()
  private let jobButton: UIButton = .init()
}
...
extension LiarGameSubjectViewController {
  func bind(reactor: LiarGameSubjectReactor) {
    ...
    reactor.state.map(\.selectedSubject)
      .compactMap { $0 }
